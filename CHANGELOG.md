...
- fix(web-gui): 内嵌打开卡在「端口未监听」——reconcile 兜底把 Starting 转 Running
- 启动探活线程只等 8s，dsh 冷启动+插件加载/pnpm 同步常超此窗口；窗口内端口未开则
- 线程退出、状态滞留 Starting，之后无人提升，导致「内嵌打开」轮询 running 超时误报
- 「端口未监听」，而端口其实已监听（浏览器可直连）。在 5s 对账循环补一条：托管进程
- 存活且端口已监听时 Starting→Running。
